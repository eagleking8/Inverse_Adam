...
class InverseAdam(Optimizer):
    def __init__(self, params, lr=1e-3, beta1=0.9, beta2=0.999, epsilon=1e-8, alpha=1.0):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                # Combined update
                p.data.add_(-lr, (1 - alpha) * adam_update + alpha * inverse_update)

                # Update alpha
                group['alpha'] = max(0.0, group['alpha'] - 0.01)  # Update alpha to gradually transition from inverse Adam to Adam
